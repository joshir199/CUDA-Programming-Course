...
// Calculate Monte Carlo Sum using given inputs sample from simulation.
// Integration = (b - a)/N  * (sum of all elements)
__global__ void monteCarloSum(float* a, float* c, float ac, float bc) {

    int tid = threadIdx.x + blockIdx.x * blockDim.x;
    extern __shared__ float cache[];

    if(tid<N) {
        cache[threadIdx.x] = (bc - ac) * a[tid] / N;
    } else {
...
int main() {
    ...
    for(int i=0; i<N ;i++) {
        h_a[i] = 1.0f;//(rand() % 90) * 0.018f - (rand() % 90) * 0.05f;
    }
    ...
    monteCarloSum<<<blockPerGrid1, threadsPerBlock, shmem>>>(d_a, d_d);
