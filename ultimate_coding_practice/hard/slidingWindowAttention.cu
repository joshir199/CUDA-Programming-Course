...
#define D 128 // common dimension  (<= 128)
#define m 512 // row for Query
#define w 32  // Window Size (<= 32)
...
__global__ void softmaxFunction(float* a, float* c, int M, int N) {
    ...
    for(int i=threadIdx.x; i<N; i+=blockDim.x) {
        ...
        cacheSum[i] = (val == -FLT_MAX) ? 0.0f : expf(val - blockMax[0]);  // set masked element to 0
        localSum += cacheSum[i];
    }
    __syncthreads();
...
__global__ void matMul(float* a, float* b, float* c, int M, int d, int N, int W, bool scaled) {
    ...
    if(scaled) { // If scaled matrix multiplication as in attention
        // To Implement Sliding Window Self-Attention of window size W, where each query i only attend to [i-W, i+W] Keys
        // It uses a causal mask that sets all positions outside of the Window to -INFINITY which will be 0 during softmax
        // Include casual mask in QK_T, QK_T[row][col] = -INFINITY { abs|row (query) - col (key)| > W }
        // and for other cases, it will be as usual
        if(x_c< N && y_r < M) {
            //To avoid warp divergence, you can compute the same result branchlessly (to avoid if())
            float casual_val = (fabsf(x_c - y_r) > W) ? -FLT_MAX : partialsum * rsqrtf((float)d);
...
int main() {
    ...
    matMul<<<grid1, block1>>>(d_q, d_kt, d_qk, M, d, N, W, true);
    CHECK_CUDA(cudaGetLastError());
    CHECK_CUDA(cudaDeviceSynchronize());
    ...
    matMul<<<grid2, block2>>>(d_qks, d_v, d_c, M, N, d, W, false);
    CHECK_CUDA(cudaGetLastError());
    CHECK_CUDA(cudaDeviceSynchronize());
    ...
    cout<<"ELapsed time(in ms) is "<<elapsed_time<<endl; //0.46
