...
// softmax function for matrix A[MxN]
__global__ void softmaxFunction(float* a, float* c, int M, int N) {
    ...
    float localMaxVal = -FLT_MAX; // per thread
    for(int i=threadIdx.x; i<N; i+=blockDim.x) {
    ...
    // Now, we will use atomic to get per block sum
    atomicAddFloat(&blockSum, localSum); // each thread in a block will add to simgle blockSum
    __syncthreads();

    // Now, make it per thread:
    float totalSumPerRow = blockSum;
    ...
    for(int i=threadIdx.x; i<N; i+=blockDim.x) {
       ...
       c[i + rowOffset] = cacheSum[i] / fmaxf(totalSumPerRow, FLT_EPSILON);
    }
}
