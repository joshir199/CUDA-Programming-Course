...
int main() {
    ...
    cudaEvent_t start, stop; // define the event variable
    cudaEventCreate(&start); // create the start variable for event
    cudaEventCreate(&stop);  // create the stop variable for event
    cudaEventRecord(start, 0); //start recording the start time of the event
    ...
    cudaEventRecord(stop, 0); // start recording the time of end of the event
    cudaEventSynchronize(stop); // ensure the no new event starts before previous events ends

    float time_elapsed;
    cudaEventElapsedTime(&time_elapsed, start, stop); // calculate the event time

    cout<< "time (in ms) elapsed for the event: " << time_elapsed <<endl;

    cudaEventDestroy(start); // destroy the variable for events
