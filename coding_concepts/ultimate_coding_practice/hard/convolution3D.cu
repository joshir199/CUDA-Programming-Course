...
__global__ void conv3d_kernel(float* input, float* kernel, float* output, int input_deps, int input_rows, int input_cols, int k_deps, int k_rows, int k_cols)
{
    // valid output size
    ...
    for (int d = 0; d < k_deps; d++) {
        for (int i = 0; i < k_rows; i++) {
            for (int j = 0; j < k_cols; j++) {
                ...
                convVal += input[gIdx] * kernel[kIdx];
            }
        }
    }
...
int main() {
    ...
    CHECK_CUDA(cudaMalloc(&d_b, K_D*K_R*K_C*sizeof(float)));
    ...
    CHECK_CUDA(cudaMemcpy(d_b, h_b, K_D*K_R*K_C*sizeof(float), cudaMemcpyHostToDevice));
    ...
    conv3d_kernel<<<grid, block>>>(d_a, d_b, d_c, D, M, N, K_D, K_R, K_C);
    ...
    cout<<"GPU Elapsed time(in ms) : "<< elapsed_time<<endl;  // 1.58 for all 64x128x128
