...
__global__ void vectorAdd(int* a, int* b, int* c, int n) {
    ...
    while(tid<n) {
        c[tid] = a[tid] + b[tid];
        tid += blockDim.x * gridDim.x; // increment by num of parallel threads in a grid
    }
}
...
int main() {
    ...
    //Here, we will provide only few blocks and threads withing the limit
    // We should understand that GPU does not need to do computation at one go,
    // It can use parallel programming and use few operation steps.
