...
// Calculate Sparse Matrix - Vector Multiplication with matrix in CSR format
...
// Convert Sparse matrix into its CSR format with Column array, Values array and row pointers
...
int main() {
    ...
    cout<<"ELapsed time(in ms) is "<<elapsed_time<<endl; //1.49
